			...
			<Header>
				<template #header-layout>
					<div class="header-layout">
						<Breadcrumb label="Perfil" @back="goBack" />
						<div class="buttons">
 ...
 */
const getUser = async () => {
	const user = route.query.id;
	if (!user) return;

	try {
		const { data } = await api.setMethod('get').setEndpoint(`users/${user}`).send();
		name.value = `${data.user.fname} ${data.user.lname}`;
