<template>
    <div class="check-list">
        <div class="check-button-container" v-for="(button, index) in buttons" :key="index"
            :class="{ selected: button.selected }" @click="toggleSelection(index)">
            <div class="check-icon" :class="{ selected: button.selected }">
                <svg v-if="button.selected" width="16" height="16" viewBox="0 0 16 16" fill="none"
...
const props = defineProps({
    buttons: {
        ...
        validator: (value) => {
            return value.every(
                (item) => typeof item.option === 'string' && typeof item.selected === 'boolean'
            );
        },
    },
});
